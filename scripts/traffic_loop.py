...
def main() -> None:
    ...
    p.add_argument("--schema-version", type=int, default=1, help="Predict API schema_version to send")
    ...
    while True:
        ...
        label_value = y.iloc[idx]
        actual_label = int(label_value) if pd.notna(label_value) else None
        try:
            requests.post(
                f"{args.url}/predict",
                json={
